...
#### A. Cleaning: Drop samples with <300 individuals; subsample those with >300 individuals to 300 individuals: 
...
Drop these samples from the dataset: 

```{r}
density_eco_300 <- density_eco %>%
  filter(!sampleId%in%under300$sampleId)
...
eco_meta <- density_eco_300%>%
  select(1:4, 21:37, -taxa_density)%>% #select metadata columns from ecoregion density DF
...
Calculate density, then count # unique species and calculate avg. density in each ecoregion in each year: 

```{r}
eco_r_d <- subsets_eco %>%
  mutate(density = (splitCount/(labSplit*fieldSplit))/area)%>% #Density = # individuals/ls*fs / area sampled
  ...
  summarise(ecoCode = unique(ecoCode), #add ecoregion code back in
            ...
            density_xbar = mean(density), #calculate mean density
