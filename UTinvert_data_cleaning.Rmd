...
First, need to calculate richness and total density for each sample: 

```{r}
ecoSite_means <- subsets_eco2 %>%
  group_by(sampleId)%>% #group by sample ID
  ...
  group_by(siteId, yr)%>% #group by siteId and year
  summarise(
    dens_xbar = mean(tot_dens), 
    richness_xbar = mean(richness), #calculate mean density and richness for each site*year
    ecoName = unique(ecoName) #retain ecoregion name
  )
```

**NOTE** - because we subsampled to 300 individuals, the total density at each site will be the same for sites with the same lab split/field split/sample areas - so may not be a useful metric. 
...
```{r}
dens.eco.site <- ggplot(ecoSite_means, aes(x = yr, y = log10(dens_xbar), group = factor(siteId), color = ecoName))+
  geom_point()+
  geom_line()+
  facet_wrap(~ecoName)+
  theme_classic()+
...
```{r}
rich.eco.site <- ggplot(ecoSite_means, aes(x = yr, y = log10(richness_xbar), group = factor(siteId), color = ecoName))+
  geom_point()+
  geom_line()+
  facet_wrap(~ecoName)+
  theme_classic()+
...
```{r}
write.csv(eco_r_d, "cleaned_data/eco_richness_density.csv")
```
...
```{r}
statewide_10plus <- subsets_eco %>%
  group_by(yr)%>% #group by year
  mutate(n_sites = length(unique(siteId)))%>% #count n unique sites surveyed in that year
  ungroup()%>% #ungroup to retain all columns
  filter(!n_sites<10) #drop all years with fewer than 10 sites
```

#### B. Calculate state-wide density and richness by year:

Group by year and count unique taxa, calculate mean total sample density for each year:

```{r}
statewide_yearly <- statewide_10plus %>%
  group_by(yr)%>%
  summarise(richness = length(unique(scientificName)), 
            dens_xbar = mean(tot_dens), 
            n_sites = unique(n_sites), 
            richness_persite = richness/n_sites, 
            dens_persite = dens_xbar/n_sites)
```
